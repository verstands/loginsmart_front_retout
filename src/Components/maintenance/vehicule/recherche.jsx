...
const Recherche_maintenace = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`${process.env.REACT_APP_SERVICE_API}vehicule/${siteSession}`,
            ...
            alert(error + "vehicule")
        })
    }, [])

    useEffect(() => {
        axios.get(`${process.env.REACT_APP_SERVICE_API}fseurT`,
            {
                headers: {
                    Accept: 'application/json',
                    'Content-Type': 'application/json',
    ...
    const [tyepeR, settypeR] = useState([]);
    useEffect(() => {
            ...
            <div className="card">
                <div className="card-body">
                    <div className="row">
                        ...
                        <div className="col-md-3">
                            <label htmlFor="">Numero reparation</label>
                        ...
                        <div className="col-md-3">
                            ...
                            <select onChange={(e) => setchauffeurs(e.target.value)} id="" className="form-control">
                                    <option value="">--Choisir Fseur---</option>
                                {
                                    chauffeur.map((chauf) => {
                                        return (
                                            <>
                                                <option key={chauf.id} value={chauf.id}>{chauf.nom}</option>
                                            </>
                                        )
                                    })
                                }
                        ...
                        <div className="col-md-3">
                            <label htmlFor="">Type rep.</label>
                            <select onChange={(e) => setquantites(e.target.value)} className="form-control">
                                {
                                    tyepeR.map((vill) => {
                                        return (
                                            <option value={ville.id} key={vill.id}>{vill.type_rep}</option>
                                        )
                                    })
                                }
                            </select>
                                ...
                                searchs.map((shs) => {
                                    return (
                                        <tr>
                                            ...
                                            <td>
                                                <button className="mdi mdi-apps btn btn-primary"></button>&nbsp;
                                            </td>
                                        </tr>
                                    )
                                })
...
export default Recherche_maintenace