...
const Vehicule_site3week = ({dataLineAMois}) => {
  ...
  const dataSitesLineMois = {
    ...
    datasets: Array.isArray(dataLineAMois) && dataLineAMois.length > 0 ? (() => {
      ...
      return immatriculations.map((nom_site) => {
        const filteredData = Array.isArray(dataLineAMois) ? dataLineAMois.filter((item) => item.id === nom_site) : [];
...
export default Vehicule_site3week